...
export class DefaultBlockSource extends IRuleSource {
  ...
  async fetchRules() {
    try {
      ...
      console.log(`✅ Fetched ${this.name} from GitHub`);
      return await response.json();
    } catch (error) {
      // Fallback to bundled local file if GitHub fetch fails
      console.warn(`⚠️ GitHub fetch failed for ${this.name}: ${error.message}`);
      console.log(`🔄 Falling back to bundled default-block-requests.json`);
      ...
      try {
        ...
        console.log(`✅ Loaded ${this.name} from bundled file`);
